...
export function Polaroid({
  ...
  const style = {
    ...
    "--hover-rotation": `${-Math.sign(image.rotation ?? 0) * 0.1}deg`,
    "--position-x":
      image.position?.x === undefined ? "50%" : toCssLength(image.position.x),
    "--position-y":
      image.position?.y === undefined ? "50%" : toCssLength(image.position.y),
    "--polaroid-width":
      image.position?.width === undefined
        ? "42%"
        : toCssLength(image.position.width),
  } as CSSProperties;

  return (
    <button
      type="button"
      className={`polaroid ${image.className ?? ""}`}
      style={style}
      onClick={onOpen}
      aria-label={`Pick up ${image.alt} to view it larger`}
    >
      ...
      <div className="collage" aria-label="Photo collage">
        {images.map((image) => (
          <Polaroid
            key={image.id}
            image={image}
            onOpen={(event) => openPhoto(image, event)}
          />
        ))}
      </div>
      {selected &&
